...
describe('xlsxToTransactions', () => {
  it('reads every sheet and maps Debet->income, Kredit->expense', () => {
    const { rows, skipped } = xlsxToTransactions(buildWorkbook(), categories, accounts);
  ...
  it('converts Excel date cells to YYYY-MM-DD and resolves categories', () => {
    const { rows } = xlsxToTransactions(buildWorkbook(), categories, accounts);
    ...
    expect(expense.account_id).toBe('a1');
    ...
    expect(income.account_id).toBe('a1');
  });
  ...
  it('returns empty result for a workbook without a Tanggal header', () => {
    ...
    const { rows } = xlsxToTransactions(buf, categories, accounts);
    expect(rows).toHaveLength(0);
  });

  it('maps the Bank column to a known account or a custom account', () => {
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(
      wb,
      XLSX.utils.aoa_to_sheet([
        HEADER,
        [1, new Date(2026, 8, 5), 'Top up', '', 50000, '', 'Dana'],
        [2, new Date(2026, 8, 6), 'Bakso', '', 25000, 'Makanan', 'GoPay'],
      ]),
      'September'
    );
    const buf = XLSX.write(wb, { type: 'array', bookType: 'xlsx' }) as Uint8Array;
    ...
    const dana = rows.find((r) => r.description === 'Top up')!;
    expect(dana.account_id).toBe('a2');
    expect(dana.custom_account).toBe('');

    const gopay = rows.find((r) => r.description === 'Bakso')!;
    expect(gopay.account_id).toBe('');
    expect(gopay.custom_account).toBe('GoPay');
  });
});
